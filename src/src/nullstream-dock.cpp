...
void updateStreamButton()
{
	...
	g_streamBtn->setText(active ? QStringLiteral("\u25A0 配信を終了")
				    : QStringLiteral("\u25CF 配信を開始"));
}
...
QWidget *buildDockWidget()
{
	...
	QPushButton *refreshBtn =
		new QPushButton(QStringLiteral("更新"), root);
	...
	QObject::connect(g_sceneList, &QListWidget::itemClicked, root,
	...
	QObject::connect(g_coverBtn, &QPushButton::clicked, root, []() {
		if (!g_coverOn) {
			...
			if (!g_sceneBeforeCover.empty())
				switchToScene(QString::fromUtf8(
					g_sceneBeforeCover.c_str()));
			g_coverOn = false;
